...
| FFmpeg engine — `ExtractAudio`/`ExtractFrames` | **Deferred, and now declared as such.** Still on `IMediaEngine`, still throws `E_NOT_IMPLEMENTED` — but `getCapabilities` no longer advertises them as ordinary capabilities. They come back in `deferredCapabilities` with a user-facing reason instead, so the frontend can disable a control rather than offer it and fail. One table (`core/media/DeferredOperations.h`) feeds both the advertisement and the error. |
